...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, num_layers=1):
        ...
        # embedding layer: To turn words into a vector form
        self.word_embeddings = nn.Embedding(vocab_size, embed_size)

        # LSTM Layer: Takes in embedded word vectors as inputs and outputs hidden states of size hidden_dim
        self.lstm = nn.LSTM(input_size=embed_size,
                            hidden_size=hidden_size,
                            num_layers=num_layers,
                            batch_first=True)

        # Linear layer: To map the hidden state output dimension to the vocab_size
